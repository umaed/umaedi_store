...
(function () {
  ...
  function mergeFirebaseOrders(orders) {
    if (!Array.isArray(orders) || orders.length === 0) return;

    const localOrders = getHistory();
    const remoteById = new Map(orders.map(order => [order.id, order]));
    const merged = localOrders.map(order => remoteById.get(order.id) ? { ...order, ...remoteById.get(order.id) } : order);
    orders.forEach(order => {
      if (!merged.some(localOrder => localOrder.id === order.id)) merged.push(order);
    });
    saveHistory(merged.sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0)));
    render();
  }
  ...
  if (window.firebaseStore) {
    window.firebaseStore.subscribeOrders(mergeFirebaseOrders);
  } else {
    window.addEventListener('load', () => window.firebaseStore?.subscribeOrders(mergeFirebaseOrders), { once: true });
  }
})();
