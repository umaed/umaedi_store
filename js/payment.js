...
function publishPaymentOrder(order) {
  if (!window.firebaseStore) return;
  window.firebaseStore.saveOrder(order);
}
...
function buildPaymentMessage(session, totals, proofName) {
  ...
  if (productItems.length > 0) {
    ...
    productItems.forEach((item, index) => {
      ...
      // If this is a follower package, include follower count and target account
      if (item.followers) {
        lines.push(`   Followers : ${item.followers}`);
      }
      if (item.targetAccount) {
        lines.push(`   Target    : @${item.targetAccount}`);
      }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Update sticky total display and animate it
  const stickyTotal = document.getElementById('sticky-total');
  const stickyWrap = document.querySelector('.sticky-total');
  function updateStickyAmount(amount) {
    if (!stickyTotal) return;
    stickyTotal.textContent = paymentRupiah(amount);
    // trigger animation class
    if (stickyWrap) {
      stickyWrap.classList.remove('active');
      // reflow to restart animation
      void stickyWrap.offsetWidth;
      stickyWrap.classList.add('active');
    }
  }

  updateStickyAmount(totals.total);
