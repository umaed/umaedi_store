...
function renderProductDetail() {
  ...
  root.innerHTML = `
    <section class="product-detail">
      ...
      <div class="detail-content">
        ...
        ${product.unavailable ? `<div class="detail-alert">Perhatian: ${product.notice}</div>` : ''}
  ...
  document.getElementById('detail-add').addEventListener('click', event => {
    ...
    if (product.selectType === 'redirect') {
      window.location.href = product.link || getProductDetailUrl(product.id);
      return;
    }
    if (product.selectType === 'variant') {
