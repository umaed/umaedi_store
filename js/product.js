    ...
    <div class="product-info">
      ...
      <div class="product-actions">
        <button class="product-btn ${product.unavailableLabel ? 'has-clock' : ''}" data-id="${product.id}" data-type="${product.selectType}" ${product.unavailable ? 'disabled' : ''}>
          ${product.unavailable ? product.unavailableLabel || 'Ditutup Sementara' : product.selectType === 'redirect' ? 'Lihat Detail' : product.selectType === 'variant' ? 'Pilih Varian' : 'Tambah'}
        </button>
  ...
  section.querySelectorAll('.product-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (action === 'favorite') {
        ...
        const prefix = window.location.pathname.includes('/pages/') ? '' : 'pages/';
        window.location.href = `${prefix}follower-variants.html`;
      } else {
